...
def convert_bool_to_int(value):
    try:
        ...
        logging.error("Error to convert bool to int")
    finally:
        return value
...
def convert_anything_to_bool(value):
    ...
    try:
        ...
        logging.error("Error to convert anything to bool ")
    finally:
        return boolvalue
...
def nan2floatzero(value):
    try:
        ...
        logging.error("Error to convert NaN value to 0.00 - {}".format(e))
...
def sshCommand(command, hostname, username, tcpport=22, password=None, pkey=None):
    ...
    try:
        ...
        logging.debug("ssh connected on host {}".format(hostname))
        ssh.connect(**kword)
...
def df_to_picke(df, file):
    try:
        ...
        logging.error("Error to save dataframe to pickle file {} - {}".format(file,e ))
...
def picke_to_dataframe(file):
    if check_is_file_exist(file):
        try:
            ...
            logging.error("The file exist but I can convert picke file to dataframe")
    else:
        logging.error("Error with file, maybe it's not existe or cannot open")
